...
class TaggingView(ctk.CTkFrame):
    ...
    def build_main_panel(self):
        ...
        self.print_batch_btn = ctk.CTkButton(search_frame, text="⎙ Print Selected QRs", width=150, fg_color="#2ECC71", hover_color="#27AE60", text_color="white", font=("Inter", 11, "bold"), command=self.print_selected_qrs)
        self.print_batch_btn.pack(side="right", padx=10)

        self.headers = ["Select", "PID", "Type", "Name", "Qty", "Location", "Status", "Tag ID"]
    ...
    def toggle_select_all(self):
        if not hasattr(self, 'select_all_var'): return
    ...
    def load_tagging_data(self, query="", filter_type="All Tools"):
        ...
        for col, text in enumerate(self.headers):
            ...
            if col == 0:
                chk_all = ctk.CTkCheckBox(cell, text="", variable=self.select_all_var, command=self.toggle_select_all, width=20, border_color="white", checkmark_color="#1E4528")
        ...
        try:
            ...
            for i, row_data in enumerate(results):
                ...
                display_data = ["", str(tool_id), str(item_type), str(name), qty_str, str(loc), str(cond), str(tag)]
                full_data = [tool_id, name, desc, price, qty_avail, loc, cond, tag, cat, sup]
                ...
                for col, val in enumerate(display_data):
                    ...
                    if col == 0:
                        var = ctk.BooleanVar(value=tool_id in self.selected_tool_ids)
                        self.chk_vars[tool_id] = var
                        cmd = lambda v=var, tid=tool_id: self.selected_tool_ids.add(tid) if v.get() else self.selected_tool_ids.discard(tid)
                        chk = ctk.CTkCheckBox(cell, text="", variable=var, command=cmd, width=20)
                        chk.pack(expand=True, pady=10)
                    else:
                        txt_col = "#1A1A1A"
                        if col == 7 and val == "Unassigned": txt_col = "#D8000C"
                        elif col == 2 and val == "Consumable": txt_col = "#D37F00"
                        
                        font_w = "bold" if (col == 7 and val != "Unassigned") or col == 2 else "normal"
    ...
    def print_selected_qrs(self):
        if not self.selected_tool_ids:
            messagebox.showwarning("Warning", "No tools selected for batch printing.", parent=self.winfo_toplevel())
            return
        ...
        try:
            ...
            if not images:
                messagebox.showwarning("Warning", "Selected tools do not have Tag IDs assigned. Please assign tags first.", parent=self.winfo_toplevel())
                return
    ...
    def open_tag_manager(self, data):
        ...
        ctk.CTkLabel(form_frame, text="Assigned Tag ID / QR Code", font=("Inter", 12, "bold"), text_color="#1A1A1A").pack(anchor="w")
        ...
        def update_preview():
            ...
            ctk.CTkButton(preview_frame, text="⎙ Print QR", width=140, height=35, fg_color="#1E4528", hover_color="#14301C", font=("Inter", 12, "bold"), command=execute_print).pack(pady=(5, 10))
