...
class RoleManagementView(ctk.CTkFrame):
    ...
    def execute_register(self):
        ...
        if role == "Worker":
            ...
            # --- FIX A: Email is now strictly required for Staff/Admin ---
            if not all([name, email, pwd, cpwd]):
                return messagebox.showerror("Validation Error", "Full Name, Email, and Password are required.", parent=self.winfo_toplevel())
            if pwd != cpwd:
