...
class ProjectsView(ctk.CTkFrame):
    ...
    def open_draft_project_modal(self, edit_row=None, reqs=None):
        ...
        if edit_row:
            ...
            if is_locked:
                ...
                self.p_start.configure(state="disabled", fg_color=lock_color)
                self.p_end.configure(state="disabled", fg_color=lock_color)
                # Disable the calendar buttons and tool picker
                for w in form_card.winfo_children():
                    if isinstance(w, ctk.CTkButton) and w.cget("text") in ("📅", "🔍 Browse Inventory Catalog"):
                        w.configure(state="disabled", fg_color="#CCCCCC")
                # Disable worker add/scan buttons
                for w in worker_input_row.winfo_children():
                    if isinstance(w, ctk.CTkButton):
                        w.configure(state="disabled", fg_color="#CCCCCC")
                self.worker_single_entry.configure(
                    state="disabled", fg_color=lock_color)
                ctk.CTkLabel(form_card, text="🔒 Client, Head, Location, Dates, Workers & Tools are locked for confirmed projects.",
    ...
    def scan_worker(self):
        ...
        if not cap.isOpened():
            return messagebox.showerror("Camera Error", "No webcam detected.", parent=self.winfo_toplevel())
    ...
    def save_project(self, publish=True):
        ...
        if not name or not client:
            return messagebox.showerror("Error", "Project Name and Client are required.", parent=self.winfo_toplevel())
        ...
        if start_date and end_date:
            if datetime.strptime(end_date, "%Y-%m-%d") < datetime.strptime(start_date, "%Y-%m-%d"):
                return messagebox.showerror("Invalid Date Range", "End Date cannot be earlier than Start Date.", parent=self.draft_modal)
        ...
        try:
            ...
            if editing_id:
                old_status = getattr(self, "editing_project_status", "Draft")
                raw_old = old_status.replace(' (OVERDUE)', '')

                # Locked statuses: only allow name/description edits
                if raw_old in ('Approved', 'Ongoing'):
                    new_status = raw_old
                    cursor.execute('''
                        UPDATE projects
                        SET name=%s, description=%s, status=%s
                        WHERE project_id=%s
                    ''', (name, desc_text, new_status, editing_id))
                else:
                    new_status = raw_old
                    if raw_old == 'Draft' and not self.is_admin:
                        new_status = 'Draft'  # Stays draft on update unless published
                    cursor.execute('''
                        UPDATE projects
                    ...
                    for item in self.req_cart:
                        ...
                        cursor.execute('''
                            ...
                            VALUES (%s, %s, %s, %s)
                        ''', (editing_id, item['tool_id'], item['qty'], req_status))

                project_id = editing_id
                action_text = "Updated"
            else:
                # New project: Draft or Approved (published)
                new_status = 'Approved' if publish else 'Draft'
                approved_by_val = self.user_info['user_id'] if publish else None
                cursor.execute('''
                ...
                ''', (name, desc_text, project_head, client, self.p_location.get(), workers_str, start_date, end_date, self.user_info['user_id'], new_status, approved_by_val))
    ...
    def build_table_panel(self):
        ...
        self.proj_search = ctk.CTkEntry(
            top, placeholder_text="Search project or client...", width=250, takefocus=True, textvariable=self.search_var)
        ...
        def on_search_change(*args):
            ...
            self._search_timer = self.after(
                300, lambda: self.load_projects(self.proj_search.get().strip()))
            ...
            for i, row in enumerate(results):
                ...
                for col, (val, wrap_l) in enumerate(vals):
                    ...
                    lbl = ctk.CTkLabel(cell, text=val, font=(
                        "Inter", 11, font_w), text_color=txt_color, justify="left", anchor="w")
